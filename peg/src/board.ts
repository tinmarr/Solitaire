class Board {
  board: Phaser.GameObjects.Sprite;
  gamePieceClasses: Piece[];
  gamePieceSprites: Phaser.Physics.Arcade.Sprite[];
  numLeft: integer;
  constructor() {
    ...
    for (let i = 0; i < 49; i++) {
      let pos = { x: i % 7, y: Math.floor(i / 7) },
        visible = false,
        piece;
      if (1 < pos.y && pos.y < 5) {
        visible = true;
  ...
  isLegalMove(pieceClass, newPos) {
    if (pieceClass.pos.ideal.x !== newPos.x && pieceClass.pos.ideal.y === newPos.y) {
      return (
        Math.abs(pieceClass.pos.ideal.x - newPos.x) === 2 &&
        !this.isPosEmpty({
      ...
      return (
        Math.abs(pieceClass.pos.ideal.y - newPos.y) === 2 &&
        !this.isPosEmpty({
          x: newPos.x,
